...
class BHL_Download:
    ...
    def __init__(self, donwload_path):
...
if __name__ == '__main__':
    ...
    parameter_list = ['', 'xml path', 'replace item', 'replace title', 'start date = iso date | start | current', 'end date = iso date | current ' ]
    parameters = Parameters(parameter_list)
    if parameters.check_parameters(sys.argv):
        
        from datetime import  date, timedelta
        curr_date = datetime.today().isoformat()[0:10]
        next_date = date.today() + timedelta(days=1)
        next_date = next_date.isoformat()[0:10]

        script_name, xml_path, rep_item, rep_title, start_date, end_date = sys.argv
        
        proc = BHL_Download(xml_path)
